...
# Load models and preprocessors (robust path)
@st.cache_resource
def load_models():
    base_dir = os.path.dirname(__file__) or "."
    try:
        classifier = joblib.load(os.path.join(base_dir, 'best_classifier_XGBoost.pkl'))
        regressor = joblib.load(os.path.join(base_dir, 'best_regressor_XGBoost.pkl'))
        ...
        st.error(f"Model file not found: {fnf}. Ensure model files are in the repo root and committed.")
...
# Sidebar navigation (options trimmed & consistent)
...
if page == "Home":
    ...
    if classifier is None or regressor is None:
        st.error("Models not loaded. Please check model files in the repo. See logs for more details.")
        st.stop()

    # Input form (unchanged fields)
    with st.form("prediction_form"):
        ...
        if submitted:
            ...
            debt_to_income_ratio = current_emi_amount / monthly_salary if monthly_salary else 0
            expense_to_income_ratio = (monthly_rent + school_fees + college_fees + travel_expenses + groceries_utilities + other_monthly_expenses) / monthly_salary if monthly_salary else 0
            affordability_ratio = ((monthly_salary - current_emi_amount - monthly_rent - school_fees - college_fees - travel_expenses - groceries_utilities - other_monthly_expenses) / requested_amount * requested_tenure) if requested_amount else 0
            employment_stability_score = years_of_employment * 0.3 + (0.7 if employment_type == 'Government' else 0)
            financial_stability_score = (bank_balance / monthly_salary) * 0.4 + (emergency_fund / monthly_salary) * 0.6 if monthly_salary else 0
            credit_utilization = current_emi_amount / (credit_score / 100) if credit_score > 0 else 0
            dependency_ratio = dependents / family_size if family_size else 0
            ...
            # NOTE: if you used additional encodings (one-hot etc.) during training,
            # you MUST apply the same encoding here before scaling/prediction.
            try:
                # Scale features
                ...
                eligibility_proba = classifier.predict_proba(input_scaled)[0] if hasattr(classifier, "predict_proba") else None
                ...
                with col_result1:
                    if str(eligibility_pred).lower() in ['eligible', 'eligible ']:
                        st.markdown(f'<div class="prediction-result eligible">✅ EMI ELIGIBLE</div>', unsafe_allow_html=True)
                    elif str(eligibility_pred).lower() in ['high_risk', 'high-risk', 'high risk']:
                ...
                st.error(f"Input shape/columns mismatch for the scaler/model: {ve}. Make sure your input columns match training features and encoding.")
            except Exception as e:
